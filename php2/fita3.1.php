<html>
 ...
 <body>
    ...
    <form>
		<label id="valorMax">Valor Maximo </label><br>
        <input name="poblacionMax" /><br>
		<label id="valorMax">Valor Minimo </label><br>
		<input name="poblacionMin" /><br>
        <input type="submit" />
    </form>
 	<?php
        ...
        $poblacionMax = trim($_GET['poblacionMax'] ?? '');
		$poblacionMin = trim($_GET['poblacionMin'] ?? '');
        if ($poblacionMax !== '' && $poblacionMin !== '') {
            $poblacionMax_esc = mysqli_real_escape_string($conn, $poblacionMax);
			$poblacionMin_esc = mysqli_real_escape_string($conn, $poblacionMin);
            $consulta = "SELECT * FROM city WHERE Population BETWEEN '%$poblacionMin_esc%' AND '%$poblacionMax_esc%' ORDER BY Population DESC;";
        } else {
